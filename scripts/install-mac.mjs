...
const tunnelHost=process.env.POCKET_SSH_HOST||'dkz12345.com';
const hostKeyAlias=process.env.POCKET_SSH_HOST_KEY_ALIAS||'20.48.14.96';
const sshProxy=process.env.POCKET_SSH_PROXY===undefined?'127.0.0.1:7897':process.env.POCKET_SSH_PROXY;
if(!/^[A-Za-z0-9_.:-]+$/.test(tunnelHost)||!/^[A-Za-z0-9_.:-]+$/.test(hostKeyAlias))throw new Error('Invalid SSH host configuration');
if(sshProxy&&!/^[A-Za-z0-9_.:-]+$/.test(sshProxy))throw new Error('Invalid POCKET_SSH_PROXY');
if(install){
...
const trusted=execFileSync('/usr/bin/ssh-keygen',['-F',hostKeyAlias,'-f',path.join(os.homedir(),'.ssh/known_hosts')],{encoding:'utf8'});
...
const knownHostsArg=knownHosts.replaceAll(' ','\\ ');
...
const tunnelArgs=['/usr/bin/ssh','-N','-T','-o','BatchMode=yes','-o','IdentitiesOnly=yes','-o','StrictHostKeyChecking=yes','-o',`HostKeyAlias=${hostKeyAlias}`,'-o',`UserKnownHostsFile=${knownHostsArg}`,'-o','ExitOnForwardFailure=yes','-o','ServerAliveInterval=20','-o','ServerAliveCountMax=3','-o','ConnectTimeout=12'];
if(sshProxy)tunnelArgs.push('-o',`ProxyCommand=/usr/bin/nc -x ${sshProxy} -X connect %h %p`);
tunnelArgs.push('-i',key,'-R','127.0.0.1:43210:127.0.0.1:4321',`pocket-tunnel@${tunnelHost}`);
const jobs=[['com.leo.pocket-terminal', [process.execPath,path.join(root,'server/index.mjs')],env],['com.leo.pocket-terminal-tunnel',tunnelArgs,{}]];
