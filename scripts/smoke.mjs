...
try{
 ...
 const denied=await fetch(target+'/api/ticket',{method:'POST',headers:{...headers,Origin:'https://example.invalid'},body:JSON.stringify({sessionId:session.id})});if(denied.status!==403)throw new Error('Cross-origin request accepted');
 console.log('PASS: login, real Mac shell, Chinese, resize, reconnect persistence, history, anonymous/origin rejection');
