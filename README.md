...
`node scripts/install-mac.mjs` 生成专用隧道密钥与LaunchAgent草案；`--install`将运行文件复制到 `~/Library/Application Support/PocketTerminal`，再安装当前用户登录启动项（请用Node22执行）。进程以Leo普通用户运行，SSH严格验证已经信任的服务器主机密钥。当前网络默认通过 Mac 本机的 Clash SOCKS5 代理 `127.0.0.1:7897` 连接 `dkz12345.com`；如果网络可以直连 SSH，可用 `POCKET_SSH_PROXY= node scripts/install-mac.mjs --install` 禁用代理，或设置为你的本机代理地址。
